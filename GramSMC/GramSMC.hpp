...
// #define WM_BOOST_MODE              0xBD    // Performance boost (not in this
// DSDT) #define WM_ECO_MODE                0xEF    // Power saving (not in this
// DSDT) #define WM_USB_TYPEC               0x40A   // USB Type-C mode (not in
// this DSDT)
...
private:
  enum {
    kKeyboardSetTouchStatus = iokit_vendor_specific_msg(
        100), // set disable/enable touchpad (data is bool*)
    kKeyboardGetTouchStatus = iokit_vendor_specific_msg(
        101), // get disable/enable touchpad (data is bool*)
    kKeyboardKeyPressTime =
        iokit_vendor_specific_msg(110), // notify of timestamp a non-modifier
                                        // key was pressed (data is uint64_t*)
  };
  ...
  bool getTouchpadState();
  void setTouchpadState(bool enabled);
  void toggleTouchpadWithLED(); // For Fn+F5 hotkey
