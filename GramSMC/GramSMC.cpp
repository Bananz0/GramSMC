...
bool GramSMC::start(IOService *provider) {
    ...
    setProperty("Copyright", "Based on AsusSMC by Le Bao Hiep. Modified for LG Gram.");
...
IOReturn GramSMC::message(uint32_t type, IOService *provider, void *argument) {
    DBGLOG("gram", "Received message: %u Type %x Provider %s", *((uint32_t *)argument), type, provider ? provider->getName() : "unknown");
...
void GramSMC::initECDevice() {
    ...
    ecDevice = OSDynamicCast(IOACPIPlatformDevice, deviceIterator->getNextObject());
    deviceIterator->release();
...
bool GramSMC::refreshFan() {
    ...
    if (gramDevice && gramDevice->validateObject("GFAN") == kIOReturnSuccess) {
        if (gramDevice->evaluateInteger("GFAN", &speed) == kIOReturnSuccess) {
            atomic_store_explicit(&currentFanSpeed, (uint16_t)speed, memory_order_release);
...
uint32_t GramSMC::getKeyboardBacklight() {
    uint32_t level = 0;
    if (gramDevice && gramDevice->evaluateInteger("GKBL", &level) == kIOReturnSuccess) {
        return level;
    }
    return 0;
}
...
IOReturn GramSMC::postKeyboardInputReport(const void *report, uint32_t reportSize) {
...
void GramSMC::registerNotifications() {
    ...
    IOServiceMatchingNotificationHandler notificationHandler = OSMemberFunctionCast(IOServiceMatchingNotificationHandler, this, &GramSMC::notificationHandler);
...
void GramSMC::notificationHandlerGated(IOService *newService, IONotifier *notifier) {
...
bool GramSMC::notificationHandler(void *refCon, IOService *newService, IONotifier *notifier) {
    command_gate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &GramSMC::notificationHandlerGated), newService, notifier);
    return true;
}
...
void GramSMC::dispatchMessage(int message, void *data) {
    command_gate->runAction(OSMemberFunctionCast(IOCommandGate::Action, this, &GramSMC::dispatchMessageGated), &message, data);
}
...
bool GramSMC::vsmcNotificationHandler(void *sensors, void *refCon, IOService *vsmc, IONotifier *notifier) {
    if (sensors && vsmc) {
...
EXPORT extern "C" kern_return_t ADDPR(kern_start)(kmod_info_t *, void *) {
    ...
    ADDPR(debugEnabled) = checkKernelArgument("-vsmcdbg") || checkKernelArgument("-gramsmcdbg");
    return KERN_SUCCESS;
}
